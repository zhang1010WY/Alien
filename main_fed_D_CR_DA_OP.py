...
from models.local_update_radius import LocalUpdateRadius
...
if __name__ == '__main__':
    ...
    print("################################dataset_train: %d################################")
    print('dataset_train : ', len(dataset_train.x), type(dataset_train.x), type(dataset_train.labels), len(dataset_train.labels))
    ...
    for epoch in range(args.epochs):
        ...
        if epoch <= 29:
            for idx in range(args.num_users):
                local = LocalUpdateRadius(args, idx, dataset_train, train_dict_users[idx], dataset_valid_s, valid_s_dict_users[idx], dataset_valid_t, valid_t_dict_users[idx], normal_radius, abnormal_radius)
            ...
            for idx in range(args.num_dataset_segmentation):
                local = LocalUpdateRadius(args, idx, dataset_train, train_dict_users[idx], dataset_valid_s,
                                    valid_s_dict_users[idx], dataset_valid_t, valid_t_dict_users[idx], normal_radius, abnormal_radius)
        ...
        # begin mongo mongo mongo mongo mongo mongo

        base_classifier = net
        base_classifier = base_classifier.to(args.device)
        # create the smooothed classifier g
        smoothed_classifier = Smooth(args, base_classifier, 2, args.sigma)
        dataset_train_for_certify = dataset_train
        normal_radius_list_for_certify = []
        abnormal_radius_list_for_certify = []

        for i in range(len(dataset_train_for_certify)):
            ...
            (certify_flow, certify_label) = dataset_train_for_certify[i]
            certify_flow = torch.tensor(certify_flow)
            certify_label = torch.tensor(certify_label)
            certify_flow, certify_label = certify_flow.float().to(args.device), certify_label.long().to(args.device)
            prediction, radius = smoothed_classifier.certify(certify_flow, args.N0, args.N, args.alpha, args.certify_batch)
            # print('type(prediction), type(certify_label) : ', type(prediction), type(certify_label))
            correct = int(prediction == certify_label)
            # print('===============================================================')
            print('i, label, prediction, radius, correct : ', i, certify_label, prediction, radius, correct)
            # print('<><><> certify_label.item() :', certify_label.item())
            if certify_label.item() == 0.0:
                normal_radius_list_for_certify.append(radius)
            else:
                abnormal_radius_list_for_certify.append(radius)

        abnormal_radius = np.mean(abnormal_radius_list_for_certify)
        normal_radius = np.mean(normal_radius_list_for_certify)
        print('==================== abnormal_radius normal_radius ===========================', abnormal_radius, normal_radius)
