    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (flows, labels) in enumerate(self.local_data_train):
                ...
                # feature_unsuqeeze = torch.cat([encoder_flows_ori.unsqueeze(1), encoder_flows_aug.unsqueeze(1)], dim=1)
                # loss_sup = self.criterion(feature_unsuqeeze, labels_aug)

                loss_sup = 0

                result_flows = net(flows)[1]
                loss_ce = self.entropy_loss(result_flows, labels)
