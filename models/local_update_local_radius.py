...
class LocalUpdateLocalRadius(object):
    def __init__(self, args, node_id, dataset_train=None, idxs_train=None, dataset_valid_s=None, inds_valid_s=None, dataset_valid_t=None, inds_valid_t=None, normal_radius=0, abnormal_radius=0):
        self.args = args
            ...
            for batch_idx, (flows, labels) in enumerate(self.local_data_train):
                ...
                if self.args.epochs == 0:
                    ...
                    for i in range(len(dataset_train_for_certify)):
                        ...
                        print('batch_idx len dataset_train_for_certify, i,  labels[i]: ', batch_idx, len(dataset_train_for_certify), i, labels[i])
                        if i % self.args.skip != 0:
                            continue
                        if i == self.args.max:
                            break

                        (certify_flow, certify_label) = flows[i], labels[i]
                        certify_flow = torch.tensor(certify_flow)
                        certify_label = torch.tensor(certify_label)
                        certify_flow, certify_label = certify_flow.float().to(self.args.device), certify_label.long().to(self.args.device)
                        prediction, radius = smoothed_classifier.certify(certify_flow, self.args.N0, self.args.N, self.args.alpha,
                                                                         self.args.certify_batch)
                        # print('type(prediction), type(certify_label) : ', type(prediction), type(certify_label))
                        correct = int(prediction == certify_label.item())
                        print('===============================================================')
                        ...
                        if certify_label.item() == 0:
                            normal_radius_list_for_certify.append(radius)
                        else:
                            # print('certify_label.item() : ', certify_label.item(), type(certify_label.item()))
                            abnormal_radius_list_for_certify.append(radius)

                    local_abnormal_radius = np.mean(abnormal_radius_list_for_certify)
                    local_normal_radius = np.mean(normal_radius_list_for_certify)
                ...
                for idx in range(flows.size()[0]):
                    ...
                    if counterpart_idx_list.tolist():
                        ...
                        # print('========<><><>L2_distance[idx]<><><>==============', L2_distance, type(L2_distance))
                        # print('========<><><>labels[idx]<><><>==============', labels[idx].item(), type(labels[idx].item()))
                        if labels[idx].item() == 0:
                            radius_for_current = local_normal_radius
                        else:
                            radius_for_current = local_abnormal_radius
